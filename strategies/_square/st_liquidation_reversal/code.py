"""清算潮反转(多空) —— BTC 1h 单边清算潮反向:多头爆仓集中时做多,空头爆仓集中时做空;缺清算数据时不交易。

strategyId: st_liquidation_reversal  version: r1
...
(liquidation_reversal)生成;信号与仓库内置策略逐根一致(tests/standard_bot/test_square_submit.py)。不要手改。
"""
import asyncio
import time
...
from binance.strategy.node.capabilities.data import klines, liquidation_orders
...
LEVERAGE = 1
PARAMS = {'long_liquidation_threshold_usd': 100000.0,
 'short_liquidation_threshold_usd': 100000.0,
...
async def fetch_liquidations():
    return await liquidation_orders(symbol=SYMBOL, limit=1000)
...
async def execute_strategy():
    klines_raw, fetch_liquidations_raw = await asyncio.gather(fetch_klines(), fetch_liquidations())
    ctx.state["fetch_klines"] = klines_raw
    ctx.state["fetch_liquidations"] = fetch_liquidations_raw
    df = _klines_frame(klines_raw)
    df = _attach_liquidations(df, fetch_liquidations_raw)
