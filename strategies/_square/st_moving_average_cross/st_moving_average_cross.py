"""快慢均线交叉(只做多) —— BTC 1h 快慢 SMA 价差:快线高于慢线持多,低于慢线卖出,只做多现货。
...
(moving_average_cross)生成;信号与仓库内置策略的回测逐根一致(tests/standard_bot/test_square_submit.py)。不要手改。
"""
import asyncio
import time
...
_FACTOR_SPEC = {'operator': {'mode': 'emit',
              'function_name': 'ma_spread',
              'impl_source': 'def ma_spread(*, close, fast_window=5, slow_window=20):\n'
                             ...
                             "    return {'value': (fast - slow) / slow if slow else "
                             'None}\n'},
