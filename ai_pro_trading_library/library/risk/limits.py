"""Atomic-style risk-limit checks — L2 parity vs `atomic_strategy_lib.risk.limits`.

Pure-function check helpers; complement the stateful `RiskGuard` in
`library.risk.guards`. Each returns a dict so the policy layer can compose
results into `circuit_breaker_check`.

Also exposes `is_funding_window` (L1 parity vs `cyqnt_trd.blocks.risk`).
"""
...
def max_positions_check(open_count: int, max_count: int = 3) -> dict:
    """Pure-function counterpart of `RiskGuard.can_open_new` position cap."""
...
def daily_loss_check(
    ...
    daily_loss_limit_abs: float | None = None,
) -> dict:
    """Pure-function counterpart of RiskGuard daily-loss halt."""
...
def price_deviation_check(
    ...
    """Reject stale entries when live price has drifted too far."""
