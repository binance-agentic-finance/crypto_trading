"""Atomic-shape scoring helpers — L2 parity vs `atomic_strategy_lib.scoring`.

Atomic uses a different `Signal` schema (name/value/direction/strength) and
returns `Score` dataclasses with `(value, reasons, factors)`. These helpers
work on atomic-shaped inputs (duck-typed); the existing
`library.scoring.combinators` continues to operate on the canonical
`library.core.protocols.Signal` shape. The two layers are deliberately
separate.
"""
...
# ---------------------------------------------------------------------------
# Local schema (mirrors atomic_strategy_lib.core.types but standalone)
# ---------------------------------------------------------------------------
...
def additive_combine(signals: list) -> AtomicScore:
...
def majority_vote(signals: list, min_strength: float = 0.0) -> AtomicScore:
...
def hierarchical_combine(
    ...
    1. `hierarchical_combine(tiers)` where `tiers = [(name, signals, weight), ...]`
    2. `hierarchical_combine(signals, weights={...})` — delegates to weighted_composite.
    """
...
def enum_gate(value: Any, allowed, name: str = "gate") -> dict:
    """Pass/fail gate for enum-like values."""
...
def verdict_classify(
    ...
    """Classify a numeric score into a verdict string."""
    if score >= strong_min:
...
def cross_validate(scores: list) -> dict:
    """Check if multiple atomic Score objects agree on direction."""
    if not scores:
...
def conflict_detect(signals_a: list, signals_b: list) -> dict:
...
def normalize_score(
    ...
    """Linearly normalize score to target range, clamping to source range."""
