# =============================================================================
# frames.klines 的分組 —— 多時框不可以被靜默併成一條
# =============================================================================
...
# 拿到的就是這個。
#
# 修法有兩種,這裡採用的是**分組**而不是拒絕:依 (instrument, timeframe) 分成多個
# key,讓每條序列保有自己的身分。拒絕也能防止錯答案,但它會擋掉本來可以被正確服務
# 的資料;分組則兩者兼得。所以下面斷言的是「有沒有分對組」,那比「有沒有被拒絕」
# 更強 —— 它檢查的是每根 bar 有沒有落在正確的序列裡。
# =============================================================================
...
def test_two_timeframes_land_in_two_series(tmp_path):
    ...
    assert sorted(snapshot.market.bars) == ["BTCUSDT|1h", "BTCUSDT|4h"]
    assert len(snapshot.market.bars["BTCUSDT|1h"]) == 5
    assert len(snapshot.market.bars["BTCUSDT|4h"]) == 5
    for key, timeframe in (("BTCUSDT|1h", "1h"), ("BTCUSDT|4h", "4h")):
        assert {bar.timeframe for bar in snapshot.market.bars[key]} == {timeframe}, (
            "a series whose key says 1h must not contain a 4h bar — that is the "
            "concatenation this exists to stop")


def test_the_time_axis_of_each_series_is_monotonic(tmp_path):
    """The symptom the concatenation produced, asserted directly.

    Merging 1h and 4h into one list made the axis step forward, jump back 16.5
    days, then step forward again. Grouping is only a fix if each resulting
    series is individually well ordered.
    """
    rows = ([_bar("BTCUSDT", "1h", 1_784_000_000_000 + i * 3_600_000) for i in range(5)]
            + [_bar("BTCUSDT", "4h", 1_784_000_000_000 + i * 14_400_000) for i in range(5)])
    ...
    for key, series in snapshot.market.bars.items():
        times = [bar.extras["close_time"] for bar in series]
        assert times == sorted(times), key


def test_two_instruments_are_kept_apart_too(tmp_path):
    ...
    assert sorted(snapshot.market.bars) == ["BTCUSDT|1h", "ETHUSDT|1h"]
    assert {b.instrument_id for b in snapshot.market.bars["BTCUSDT|1h"]} == {"BTCUSDT"}


def test_nothing_is_dropped_while_regrouping(tmp_path):
    """Grouping must partition, not filter. 10 bars in, 10 bars out."""
    rows = ([_bar("BTCUSDT", "1h", 1_784_000_000_000 + i * 3_600_000) for i in range(5)]
            + [_bar("BTCUSDT", "4h", 1_784_000_000_000 + i * 14_400_000) for i in range(5)])

    snapshot = load_input_bundle(_write(tmp_path, _bundle(rows)))

    assert sum(len(v) for v in snapshot.market.bars.values()) == len(rows)
