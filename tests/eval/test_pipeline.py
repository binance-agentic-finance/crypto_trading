...
from cyqnt_trd.eval.engine import evaluate_factor
from cyqnt_trd.eval.snapshot import pipeline as PIPELINE
...
def test_acquisition_gap_days_override_otherwise_complete_daily_cash(daily):
    ...
    panel, detail = PIPELINE.funding_panel(index, ["A"], directory)
...
def test_midnight_plus_milliseconds_belongs_to_current_utc_day(daily):
    ...
    panel, _ = PIPELINE.funding_panel(index, ["A"], directory)
    assert panel.loc["2025-01-02", "A"] == pytest.approx(0.03)
...
def test_persisted_unknown_funding_reaches_engine_only_for_held_assets(daily, unknown_is_held):
    ...
    funding, _ = PIPELINE.funding_panel(index, ["A", "B", "C"], directory)
    ...
    result = evaluate_factor(signal, opens, opens, mask, funding,
