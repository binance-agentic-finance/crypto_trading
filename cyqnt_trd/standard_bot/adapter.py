...
def _signal_from_v2_payload(payload: Dict[str, Any]) -> StandardSignal:
    ...
    from .core import AdvisoryAction, DataQuality, TimeHorizon
    ...
    exit_plan = None
    raw_plan = payload.get("exit_plan")
    if isinstance(raw_plan, dict):
        stop = raw_plan.get("stop_loss")
        legs = tuple(
            TakeProfitLeg(close_pct=float(leg.get("close_pct") or 1.0),
                          price=leg.get("price"), pct=leg.get("pct"),
                          atr_mult=leg.get("atr_mult"), atr_value=leg.get("atr_value"))
            for leg in (raw_plan.get("take_profit") or [])
        )
        tstop = raw_plan.get("time_stop")
        exit_plan = ExitPlan(
            stop_loss=StopSpec(**{k: v for k, v in stop.items()
                                  if k in StopSpec.__dataclass_fields__}) if stop else None,
            take_profit=legs,
            time_stop=TimeStop(max_bars=tstop.get("max_bars"),
                               max_seconds=tstop.get("max_seconds")) if tstop else None,
            exit_on_opposite_signal=bool(raw_plan.get("exit_on_opposite_signal", True)),
            note=str(raw_plan.get("note") or ""),
        )

    raw_size = payload.get("size")
    size = (SizeSpec(mode=_enum(SizeMode, raw_size.get("mode"), SizeMode.EQUITY_PCT),
                     value=float(raw_size.get("value") or 0.0),
                     leverage=raw_size.get("leverage"),
                     max_notional_quote=raw_size.get("max_notional_quote"))
            if isinstance(raw_size, dict) else None)

    raw_entry = payload.get("entry")
    entry = (EntrySpec(type=_enum(EntryType, raw_entry.get("type"), EntryType.MARKET),
                       ...
                       post_only=bool(raw_entry.get("post_only")))
             if isinstance(raw_entry, dict) else None)

    prov = payload.get("provenance") or {}
    advisory = payload.get("advisory_action")
    return StandardSignal(
        bot_id=str(payload["bot_id"]), decision_time=int(payload["decision_time"]),
        provenance=Provenance(
            strategy_id=str(prov.get("strategy_id") or payload["bot_id"]),
            strategy_version=str(prov.get("strategy_version") or "v1"),
            snapshot_id=str(prov.get("snapshot_id") or ""),
            config_hash=str(prov.get("config_hash") or ""),
            inputs=tuple(prov.get("inputs") or ()),
        ),
        signal_id=str(payload.get("signal_id") or ""),
        bot_version=str(payload.get("bot_version") or "v1"),
        symbol=payload.get("symbol"), venue=str(payload.get("venue") or "binance"),
        product=str(payload.get("product") or "usd_m_perpetual"),
        market_scope=_enum(MarketScope, payload.get("market_scope"), MarketScope.SINGLE),
        intent=_enum(PositionIntent, payload.get("intent"), PositionIntent.HOLD),
        direction=_enum(Direction, payload.get("direction"), Direction.NEUTRAL),
        advisory_action=_enum(AdvisoryAction, advisory, None) if advisory else None,
        score=float(payload.get("score") or 0.0),
        confidence=float(payload.get("confidence") or 0.0),
        entry=entry, exit_plan=exit_plan, size=size,
        time_horizon=_enum(TimeHorizon, payload.get("time_horizon"), TimeHorizon.INTRADAY),
        ...
        data_quality=_enum(DataQuality, payload.get("data_quality"), DataQuality.GOOD),
