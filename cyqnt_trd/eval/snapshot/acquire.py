...
Ported from the reference study's ``acquire_data.py`` (branch
``eval-alpha101/construct``, ``eval/alpha101_crypto/``). The daily and funding
stages are unchanged in behaviour; what changed is where the snapshot lives
(``--data-dir`` instead of a directory next to the script) and two additive
stages that let a bundle carry more than daily OHLCV + funding:

``--stage klines --interval 4h``
    Complete klines at another bar width for the symbols ``universes.json``
    needs, written to ``klines_<interval>/``. Same cut-off as the daily stage
    (read from ``acquisition_config.json``), so the grids line up.
``--stage oi --interval 1h``
    Open-interest history (``futures/data/openInterestHist``), written to
    ``open_interest_<interval>/``. **The exchange serves only about the most
    recent 30 days of it**, so older cells stay missing. Run it on a schedule to
    accumulate history; each run merges with what is already on disk.

Every request goes through one rate-limited client; every file is written
atomically with a sha256 in its manifest, so an interrupted run resumes.

    python -m cyqnt_trd.eval.snapshot.acquire --data-dir eval_snapshot            # daily + funding
    python -m cyqnt_trd.eval.snapshot.acquire --data-dir eval_snapshot --stage klines --interval 4h
    python -m cyqnt_trd.eval.snapshot.acquire --data-dir eval_snapshot --stage oi --interval 1h

A re-download is a **new** snapshot: the cut-off follows the server date, so it
is not bit-identical to the one the shipped bundle was built from.
"""
...
#: openInterestHist serves the last 30 days *counted from the server's now*, not from
#: the snapshot cut-off; a startTime older than that is rejected (HTTP 400, code -1130).
OI_HISTORY_DAYS=30
_lock=threading.Lock(); _next_call=0.0


def configure(data_dir) -> Path:
    """Point every stage at ``data_dir``. Returns the resolved path."""
    global DATA
...
def api(method,url,**kw):
    import requests  # only acquisition needs the network stack
    global _next_call
...
def klines_one(sym,end,interval='1d'):
    """Complete bars at ``interval``; bars still open at the cut-off are dropped."""
    path=klines_dir(DATA,interval)/f'{sym}.parquet'
    if path.exists():
    ...
    try:
        ...
        while cur<stop:
            data=api('GET','https://fapi.binance.com/fapi/v1/klines',params={'symbol':sym,'interval':interval,'startTime':cur,'endTime':stop-1,'limit':1000})
...
def _funding_mark_open_refs(sym,event_times):
    ...
    if len(marks):
        ...
        info['cache_path']=str(cache.relative_to(DATA))
...
def funding_one(sym,end):
    ...
    try:
        ...
        result={'schema_version':3,'symbol':sym,
                ...
                'raw_path':str(raw_path.relative_to(DATA)),'raw_sha256':sha(raw_path),
        ...
        return {'symbol':sym,'status':'error','source':source,'error':str(e)[:240],
                'raw_path':str(raw_path.relative_to(DATA)) if raw_path.exists() else None}

def oi_one(sym,end,period='1h'):
    """Open-interest history at ``period``, merged into what is already on disk.

    ``ts`` is the exchange's snapshot time. The endpoint only reaches back about
    ``OI_HISTORY_DAYS`` from *now*, so for a snapshot whose cut-off is older than
    that the reachable window is short or empty; nothing older is invented, the
    builder leaves it NaN.
    """
    path=oi_dir(DATA,period)/f'{sym}.parquet'
    step=interval_ms(period)
    have=pd.read_parquet(path) if path.exists() else pd.DataFrame(columns=['timestamp','sumOpenInterest','sumOpenInterestValue'])
    stop=int(end.timestamp()*1000)
    # one hour of margin so a request built now is still inside the window when it lands
    floor=max(int(START.timestamp()*1000),now_ms()-OI_HISTORY_DAYS*86400000+3600000)
    cur=max(floor,int(have.timestamp.max())+step) if len(have) else floor
    if cur>=stop and not len(have):
        return {'symbol':sym,'status':'no_data','rows':0,'period':period,
                'reason':f'snapshot cut-off {end} is older than the exchange\'s {OI_HISTORY_DAYS}-day open-interest window'}
    try:
        rows=[]
        while cur<stop:
            part=api('GET','https://fapi.binance.com/futures/data/openInterestHist',
                     params={'symbol':sym,'period':period,'startTime':cur,'endTime':stop-1,'limit':500})
            if not isinstance(part,list):raise ValueError(str(part)[:180])
            if not part:break
            rows.extend(part)
            nxt=int(part[-1]['timestamp'])+step
            if nxt<=cur:raise RuntimeError('nonadvancing pagination')
            cur=nxt
            if len(part)<500:break
        new=pd.DataFrame(rows,columns=['timestamp','sumOpenInterest','sumOpenInterestValue'])
        ...
        if df.empty:return {'symbol':sym,'status':'no_data','rows':0,'period':period}
        bad=(df.sumOpenInterest<0)|(df.sumOpenInterestValue<0)
        if bad.any():raise ValueError(f'{int(bad.sum())} negative open-interest rows')
        df['ts']=pd.to_datetime(df.timestamp,unit='ms',utc=True)
        _funding_write_parquet(path,df)
        return {'symbol':sym,'status':'ok','period':period,'rows':len(df),'new_rows':len(new),
                'first':str(df.ts.min()),'last':str(df.ts.max()),'sha256':sha(path),
                'history_limit':f'exchange serves ~{OI_HISTORY_DAYS} days; older cells are missing, not zero'}
    except Exception as e:return {'symbol':sym,'status':'error','period':period,'error':str(e)[:240]}
...
def _fan_out(label,fn,symbols,manifest,every=25):
    results=[]
    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as pool:
        fut={pool.submit(fn,s):s for s in symbols}
        for i,f in enumerate(concurrent.futures.as_completed(fut),1):
            r=f.result();results.append(r)
            if i%every==0 or i==len(fut):
                print(f'{label} {i}/{len(fut)} ok={sum(x["status"] in ("ok","cached") for x in results)} failures={sum(x["status"]=="error" for x in results)}',flush=True)
                dump(manifest,results)
    return results

def build_parser():
    parser=argparse.ArgumentParser(prog='python -m cyqnt_trd.eval.snapshot.acquire',description=__doc__,
                                   formatter_class=argparse.RawDescriptionHelpFormatter)
    parser.add_argument('--data-dir',default=None,help='snapshot directory (default: $CYQNT_EVAL_SNAPSHOT or ./eval_snapshot)')
    parser.add_argument('--stage',choices=['daily','funding','all','klines','oi'],default='all',
                        help="'all' = daily + funding (what the shipped bundle needs); klines/oi are additive")
    parser.add_argument('--interval',default='1h',help='bar width for --stage klines / period for --stage oi')
    return parser

def main(argv=None):
    args=build_parser().parse_args(argv)
    configure(args.data_dir or DATA)
    print(f'snapshot directory: {DATA}',flush=True)
    if args.stage in ('klines','oi'):
        interval_ms(args.interval)            # reject a malformed width before any request
        cfg_path=DATA/'acquisition_config.json'
        if not cfg_path.exists():
            raise SystemExit(f'{cfg_path} missing: run --stage daily first; klines/oi reuse its cut-off and universe')
        end=pd.Timestamp(json.loads(cfg_path.read_text())['end_exclusive'])
        union=json.loads((DATA/'universes.json').read_text())['needed_symbols']
        if args.stage=='klines':
            _fan_out(f'klines {args.interval}',lambda s:klines_one(s,end,args.interval),union,
                     DATA/f'klines_{args.interval}_manifest.json')
        else:
            _fan_out(f'open interest {args.interval}',lambda s:oi_one(s,end,args.interval),union,
                     DATA/f'open_interest_{args.interval}_manifest.json',every=10)
        print('Acquisition complete',flush=True);return
    ...
    if args.stage in ('daily','all'):
        ...
        _fan_out('daily',lambda s:daily_one(s,end),symbols,DATA/'daily_manifest.json')
    ...
    if args.stage in ('funding','all'):
        print(f'Funding histories for {len(union)} evaluated symbols',flush=True)
        res=[]
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as pool:
