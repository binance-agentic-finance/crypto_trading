...
python -m cyqnt_trd.eval score --factor cyqnt_trd.eval.examples.example_factors:reversal_5d --report-dir report --markdown card.md --json card.json
```
...
python -m cyqnt_trd.eval score --factor cyqnt_trd.eval.examples.example_factors:reversal_5d --report-dir report --bootstrap 100 --benchmark BTCUSDT

# 仅六道闸门；不产出完整诊断矩阵
python -m cyqnt_trd.eval score --factor cyqnt_trd.eval.examples.example_factors:reversal_5d --quick
```
...
默认噪声参考来自 100 条 AR(1) 随机面板，使用相同数据、dev 方向选择、执行延迟、成本和资金费路径。分位数及每项有效试验数在 [null_top10_h3.json](calibration/null_top10_h3.json) 中。
...
下列命令在仓库根目录运行，先生成新标定，再显式交给评分：

```bash
python -m cyqnt_trd.eval calibrate --trials 100 --h 5 --cost 12 --entry-lag 2 --out null_h5.json
python -m cyqnt_trd.eval score --factor cyqnt_trd.eval.examples.example_factors:reversal_5d --h 5 --cost 12 --entry-lag 2 --calibration null_h5.json --markdown card_h5.md --json card_h5.json
```
...
```bash
python -m cyqnt_trd.eval --bundle my_bundle.parquet calibrate --trials 100 --splits splits.json --out my_null.json
python -m cyqnt_trd.eval --bundle my_bundle.parquet score --factor my_ideas.py:my_factor --splits splits.json --calibration my_null.json --trials-seen 1
```
...
构建不同面板需要先取一份原始公开数据快照（约 168 MB，不随仓库提供，默认放在 `./eval_snapshot/`，已加入 `.gitignore`；也可用 `$CYQNT_EVAL_SNAPSHOT` 或 `--data-dir` 指定）。在仓库根目录运行：

```bash
python -m cyqnt_trd.eval.snapshot.acquire --data-dir eval_snapshot                     # 日线 + 逐次资金费（公开接口）
python -m cyqnt_trd.eval.snapshot.build_bundle --data-dir eval_snapshot --dry-run       # 只检查输入是否齐全
python -m cyqnt_trd.eval.snapshot.build_bundle --data-dir eval_snapshot --pool historical --out my_bundle.parquet
```

`--out` 必填：随库面板与随库标定是绑定的，重建默认不覆盖它。新面板需重新标定。

**扩展面板**（多周期、持仓量）是在同一份快照上追加取数，再建成 `Panel.extras` / 非日频格子：
...
- 非 `1d` 的面板是 `cell_scheme='regular'`：可以用 `framework.backtest_weights` / `portfolio.simulate` 回测，**不能**直接交给 `evaluate()` 与随库标定（二者要求完整的 UTC 日网格）。池子成员仍按日线快照决定，再广播到当日各根 bar。`historical` 池会用到 2021 年的预热成员，非日频时这些名字也要先取到对应周期的 K 线，缺了会直接报错。
- 持仓量来自 `openInterestHist`，**交易所只保留约最近 30 天**，更早的格子是缺失，不是 0。想要更长的历史，只能定期运行 `--stage oi` 累积（每次与磁盘上已有的合并）。对齐口径：时间戳为 `ts` 的快照视为 `ts + period` 才可得，格子 `t` 只用 bar 收盘 `t + cell` 之前可得的最新值；比 `max(period, cell)` 更旧的值不向前填充，数据源断档就留成断档。
- 重新下载是一份**新快照**：截止时间跟随服务器日期，不保证与随库面板逐位元相同。扩大历史时点标的池、引入真正封存的验证窗口、补充搜索校正和容量估计，仍是第一阶段需要补充的证据。
...
```python
from cyqnt_trd.eval.pipeline import build_strategy, cards_frame, latest_orders
...
```python
from cyqnt_trd.eval.optimize import tune
...
| 路径 | 用途 |
|---|---|
| `__init__.py` / `__main__.py` | `evaluate()`、六道闸门、14 维证据矩阵及 CLI |
| `engine.py` | 时间约定、冻结方向、IC 和完整周期现金流 |
| `bundle.py` / `provenance.py` | 输入验证、面板身份与标定契约 |
| `causality.py` | 可重跑因子的前缀一致性抽查 |
| `matrix.py` | 检查、门槛来源与裁决 |
| `targets.py` / `statistics.py` | 四目标、共同样本、日期等权关系及块 bootstrap/HAC |
| `diagnostics.py` / `plots.py` | 14 维证据盘点、秩自相关与分箱换手、八组离线图与报告 |
| `preprocess.py` | 写因子时可选的 `winsorize_mad` / `winsorize_quantile` / `zscore` / `neutralize` |
| `blueprint.py` | **策略蓝图**：触发 → 硬门 → 分层打分 → 裁决 → 方向 → 仓位 → 出场 |
| `strategy.py` | 因子合成、定仓（等名义/等风险）、滚动在线选因子 |
| `portfolio.py` | 组合模拟器：目标权重 + 现金，净额调仓、逐日资金费、组合层风控 |
| `pipeline.py` | **完整链路**：评测 → 准入 → 合成 → 权重 → 调仓买卖信号 |
| `optimize.py` | **调参**：试验账本、滚动前推、平台选择、参数集成、有效试验数/DSR/PBO |
| `baselines.py` | 五个公开基线与残差化，G5 用它判断"这只是已知风格" |
| `capability.py` | 形状适配：按标的的算子 → 面板因子，支持 `list[float]` 与 `rows` 两种输入 |
| `capability_registry.py` | **承接 capability**：manifest v3 → 面板因子，清单驱动接线、量纲归一、上游解析与一致性核对 |
| `cases.py` | **外部分层策略规格**（硬门 + 分层打分 + 裁决阈值）→ blueprint → 回测 |
| `framework.py` | **八阶段定义与编排**、任意 W 的向量化回测、无状态契约校验 |
| `bars.py` | 格子采样（time/volume/dollar/vol）、跨标的对齐、建面板 |
| `forecast.py` | dev 段冻结系数的 forecast，以及 forecast → 目标仓位 |
| `library.py` | **因子库**：Alpha101 适配、播种抽样、dev 段秩相关去重 |
| `benchmarks.py` | **基准**：买入持有、等权一篮子、现金、单因子裸用、单资产择时，及分段并排指标表 |
| `experiment.py` | **端到端实验**：候选 → 矩阵 → 准入 → 去重 → 构建 → 对比 → 报告 |
| `report.py` | 评估卡的文本与 Markdown 输出 |
| `snapshot/acquire.py` | 取原始快照：日线、逐次资金费（含标记价代理与缺口诊断），以及追加的其他周期 K 线与持仓量 |
| `snapshot/pipeline.py` | 装载快照：合约生命周期掩码、当前/时点前十两种池、资金费聚合到格子、持仓量 as-of 对齐 |
| `snapshot/build_bundle.py` | 由快照重建或扩展面板（`--interval`、`--open-interest`、`--dry-run`） |
| `examples/example_factors.py` | 可直接运行的示例因子 |
| `data/` / `calibration/` | 默认面板、capability 清单快照、来源元数据和随机信号标定 |
| `alpha101.py` | Alpha101 的 101 条公式（面板形状，横截面算子限定当时交易池） |

代码结构树、承接 capability 的实测发现，以及"构建组合 vs 买入持有 BTC / 等权一篮子 / 单因子裸用"
的完整对比结果，见 [STRUCTURE.md](STRUCTURE.md)。一条命令复现：

```bash
python -m cyqnt_trd.eval stages                       # 八个阶段与每阶段的输入输出格式
python -m cyqnt_trd.eval capabilities --rejected      # 清单里有什么、什么能跑、不能跑的原因
python -m cyqnt_trd.eval compare --source alpha101 --out report/
```

Alpha101 参考研究（取数、四组评估、诊断、增量与 REPORT.md）留在 `eval-alpha101/construct` 分支的 `eval/alpha101_crypto/`，保留其原实验口径；这里只搬了重建面板所需的取数与装载（`snapshot/`）和公式本身（`alpha101.py`）。本轮框架修复不表示重新完成了整套 Alpha101 实验；需要重跑时应明确记录新引擎、数据与标定版本。

该评估包独立于 `cyqnt.input/v1 → blocks → cyqnt.signal/v2` 交易路径，不负责生成交易指令。评分依赖 `pandas`、`numpy`、`scipy`、`pyarrow`；生成诊断图另需 `matplotlib`，快照取数（`snapshot.acquire`）另需 `requests`。在仓库根目录验证：
...
都是想重用 `cyqnt_trd/blocks/alphas/`（原 `cyqnt_trd/trading_signal/selected_alpha/`）时发现的；迁移后在本分支上复验，结果不变。不修的理由：它们属于那个模块的 owner，而且修了会改变它现有 caller 看到的数字。
...
for num in (71, 73, 77, 88, 92, 96, 101):
    fn = getattr(importlib.import_module(
        f"cyqnt_trd.blocks.alphas.alpha{num}"), f"alpha{num}_factor")
    vals = []
    for _ in range(4):
...
**2. 同目录的 `rank()` 是时间序列 rank，不是横截面 rank。** `alpha_utils.rank()` 是 `series.rank(pct=True)`，套在单一标的自己的历史上（函数收一个 `data_slice`、回一个 float）；而论文的 `rank()` 是每个日期跨宇宙取横截面。含 `rank()` 的公式（大多数）两者算的不是同一个量，值不可比，逐标的版本也产不出横截面分数——这就是 `cyqnt_trd/eval/alpha101.py` 另外存在的原因：面板形状的**第二条路，刻意新增而非取代**。
