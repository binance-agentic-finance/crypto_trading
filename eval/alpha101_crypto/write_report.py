"""Render the Chinese evaluation report from finalized v2 result tables."""
...
def image(name,alt):return f'![{alt}](results/figures/{name})'

def main():
 tags=['current_normalized','current_paper','historical_normalized','historical_paper']
 frames={t:pd.read_parquet(OUT/f'summary_{t}.parquet') for t in tags}
 summary=json.loads((OUT/'summary.json').read_text())
 diag=json.loads((OUT/'figures/current_normalized/diagnostics.json').read_text())
 def triple(tag,name,field,n=3,h=3):
  x=frames[tag];g=x[(x.factor==name)&(x.h==h)].set_index('split')
  return ' / '.join(f(g.loc[s,field],n) for s in SPLITS)
 def row(tag,name,field,split='dev'):
  x=frames[tag];return x[(x.factor==name)&(x.h==3)&(x.split==split)].iloc[0][field]
 lines=['# Alpha101 在成交额前十加密永续上的评估报告',
...
这里的“净收益”含实际历史资金费率与部分标记价格代理。少数周期资金费仍未知，因此正文收益表统一比较共同可核算周期，完整历史净收益另存为 NA。第 4 节列出排除日期及其影响。''',
 image('overview.png','101 因子在开发、验证、后段的排序信息与重点候选净收益'),
 '''## 2. 使用了哪些数据，前十如何确定
...
排序窗口为 **2026-08-11—2026-09-09**；使用 USDT 报价成交额，近似美元成交额，避免按“币的个数”比较成交量。只从当前交易中的合约选取，排除稳定币基币、指数和非 COIN 类合约。''']
 top=pd.read_csv(HERE/'data/current_top10.csv')
 lines.append(table(['名次','合约','日均成交额，十亿 USDT'],[[i+1,r.symbol,f(r.adv30_usdt/1e9,3)] for i,r in top.iterrows()]))
 lines+=['''当前前十为 BTC、ETH、SOL、ZEC、XRP、HYPE、DOGE、BNB、TRUMP、ENA。本版用实际排序得到的 TRUMP，替换旧版名单中的 PEPE。
...
单段正结果数与三段交集是不同统计，不能把它们当作同一条筛选路径。''']
 lines.append(table(['因子','冻结方向','RankIC：dev / val / oot','共同周期净 bp：dev / val / oot'],
  [[n,'+' if row('current_normalized',n,'sign')>0 else '−',triple('current_normalized',n,'ic_mean'),triple('current_normalized',n,'common_net_bp',1)] for n in ['alpha088','alpha036','alpha014','alpha040','alpha061','alpha073']]))
 lines+=['''展示 088、036、014 是因为它们通过数据条件后的开发段 IC 排名前三；040 是旧报告重点复核项；061、073 是连续正收益现象的解释项，后两者不是事前选出的赢家。

### 5.2 为什么保留 088，但暂不认定可交易
''',table(['指标，alpha088','dev','val','oot'],
 [[label]+[f(row('current_normalized','alpha088',field,s),digits) for s in SPLITS] for label,field,digits in [
  ('RankIC','ic_mean',4),('IC 的 HAC t','ic_t_hac',2),('平均信号覆盖','coverage',3),
  ('有效 IC 日期数','n_ic',0),('激活周期数，原成熟网格','n_active',0),
  ('共同周期毛收益，bp','common_gross_bp',2),('共同周期交易费用，bp','common_trading_cost_bp',2),
  ('共同周期资金费支出，bp','common_funding_bp',2),('共同周期净收益，bp','common_net_bp',2),
  ('共同周期盈亏平衡单边成本，bp','common_breakeven_bp',2)]]),
...
### 5.3 与简单基线和随机信号比较
''',table(['基线','实际冻结方向的含义','RankIC：dev / val / oot','共同周期净 bp：dev / val / oot'],
 [[n,label,triple('current_normalized',n,'ic_mean'),triple('current_normalized',n,'common_net_bp',1)] for n,label in [
 ('B_size','偏向成交额较大'),('B_lowvol10','偏向过去 10 日低波动'),('B_rev5','原反转分数取反，实际为 5 日动量'),('B_mom20','原动量分数取反，实际为 20 日反转'),('B_funding7','偏向过去 7 日低资金费')]]),
...
## 6. 换选币规则或输入后，哪些结果还在
''']
 comparison=[('current_normalized','当前前十·归一化','alpha088'),('historical_normalized','历史前十·归一化','alpha088'),
 ('current_normalized','当前前十·归一化','alpha040'),('historical_normalized','历史前十·归一化','alpha040'),
 ('current_paper','当前前十·原始输入','alpha042'),('historical_paper','历史前十·原始输入','alpha042'),
 ('current_paper','当前前十·原始输入','alpha094'),('historical_paper','历史前十·原始输入','alpha094')]
 lines.append(table(['因子','实验组','RankIC：dev / val / oot','共同周期净 bp：dev / val / oot'],
 [[n,label,triple(tag,n,'ic_mean'),triple(tag,n,'common_net_bp',1)] for tag,label,n in comparison]))
 lines+=['''历史归一化池的 088 开发段覆盖只有 49.65%，未达到 50% 数据门槛；此处仍完整列出作为已指定候选的敏感性检查。其完整可核算历史三段净收益为 −13.87 / −0.12 / −8.27 bp，共同周期表因统一删掉其他策略未知的日期而略有差异，两种口径都未给出稳定优势。
...
### 7.1 分布与四类收益目标
''',image('current_normalized/01_distribution.png','alpha088 分布、缺失与时变覆盖'),
...
''',image('current_normalized/02_four_target_bins.png','四类目标的开发段固定分箱与时间块区间'),
 '''分箱没有呈现稳定单调关系，区间普遍跨零。最高箱减最低箱的原始收益约 **−14.5 bp，95% 区间 [−98.3, 52.3]**；BTC 残差约 **+12.2 bp，区间 [−48.5, 65.0]**。这些结果不支持把分数直接校准成确定的收益幅度。

### 7.2 过去收益条件、期限与延迟
''',image('current_normalized/03_past_return_slices.png','按过去收益分桶后的预测关系'),
 '''按过去收益分桶后，原始和残差目标的 Pearson 相关、斜率区间均跨零。旧版“某个上涨切片已有效”的说法不再保留。
''',image('current_normalized/04_horizon_and_delay.png','持有期限与额外等待时间的分段敏感性'),
...
### 7.3 年度稳定性与极端案例
''',image('current_normalized/05_annual_candidates.png','仅按开发段选出的三个候选的年度表现'),
 '''年度均值显示表现并不均匀。完整年度净收益只要有未知周期就记为 NA，图中的空格不能理解为零收益。
''',image('current_normalized/06_extreme_cases.png','alpha088 极端信号的正确和错误案例'),
...
## 8. 基线控制与本轮裁决
''']
 inc=OUT/'incremental_current_normalized_h3/metrics_common_signal_sample.csv'
 if inc.exists():
  lines += ['''另做了一个事后增量诊断：将 088、040 与五个基线限制在共同有效的资产和日期，按开发段拟合因子排名对基线排名的线性投影，再将系数冻结到后两段。模型只拟合分数、不拟合未来收益；未优化组合权重。
...
投影会改变有效样本和开发段方向，不能与首页原因子指标直接相减，也不能把残差指标当成已验证的组合收益。原方向残差 IC 与重新冻结方向后的交易结果分别保存，见[方法与完整比较](results/incremental_current_normalized_h3/METHOD.md)。''']
  im=pd.read_csv(inc)
  ir=[]
  for name in ['alpha088','alpha040']:
   a=im[im.factor==name].set_index('split');b=im[im.factor==name+'_residual'].set_index('split')
   ir.append([name,' / '.join(f(a.loc[s,'ic_at_primary_or_projection_orientation'],4) for s in SPLITS),
     ' / '.join(f(b.loc[s,'ic_at_primary_or_projection_orientation'],4) for s in SPLITS)])
  lines += [table(['同一共同样本，dev / val / oot','原分数 IC','保持原方向的投影残差 IC'],ir),
